---
title: "OpenManus & OpenManus-RL"
excerpt: "Core author of OpenManus-RL in an open-source agent ecosystem with 70,000+ GitHub stars."
collection: portfolio
permalink: /projects/openmanus-rl/
date: 2025-01-01
link: "https://github.com/OpenManus/OpenManus-RL"
github: "https://github.com/OpenManus/OpenManus-RL"
stars: "4.1K"
showcase_group: agent-systems
badge: "Core Author"
date_label: "Jan 2025 – Jul 2025"
tags:
  - "ReAct Agents"
  - "Multi-Agent Systems"
  - "MCP"
  - "SFT / RL"
---

OpenManus-RL is a verl-based agent RL post-training framework and the follow-up project of OpenManus. I collaborated with the official OpenManus team as a core author in an open-source ecosystem with 70,000+ GitHub stars.

Key contributions include:

- Connected SFT, RL (PPO/GRPO), interactive environments, and trajectory rollout into one unified pipeline for training ReAct agents and tool use.
- Targeted long-horizon tasks such as GAIA, WebShop, and ALFWorld.
- Unified multi-turn environment interaction, tool actions, validation, and reward under a single ReAct trajectory protocol.
- Released a dataset of ~49K high-quality agent trajectories spanning OS, Web, Database, Knowledge Graph, Household, and E-commerce scenarios.
